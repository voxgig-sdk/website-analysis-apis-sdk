...
module WebsiteAnalysisApisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WebsiteAnalysisApisResponse.new({})
    ...
    if fetch_err
      ...
      response = WebsiteAnalysisApisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WebsiteAnalysisApisResponse.new(fetched)
    else
