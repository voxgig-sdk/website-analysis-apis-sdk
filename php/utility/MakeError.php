...
class WebsiteAnalysisApisMakeError
{
    public static function call(?WebsiteAnalysisApisContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WebsiteAnalysisApisContext([], null);
        }
        $op = $ctx->op ?? new WebsiteAnalysisApisOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WebsiteAnalysisApisResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WebsiteAnalysisApisError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WebsiteAnalysisApisError('', $msg, $ctx);
        ...
        if ($err instanceof WebsiteAnalysisApisError) {
