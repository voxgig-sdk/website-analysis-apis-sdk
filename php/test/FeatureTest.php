...
// WebsiteAnalysisApis SDK feature test
//
...
class FtCtrl extends WebsiteAnalysisApisControl
{
...
class FtClient
{
    ...
    private WebsiteAnalysisApisUtility $utility;

    public function __construct(WebsiteAnalysisApisUtility $utility, array $options)
    {
    ...
    public function get_utility(): WebsiteAnalysisApisUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WebsiteAnalysisApisContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WebsiteAnalysisApisContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WebsiteAnalysisApisContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WebsiteAnalysisApisConfig::make_feature($name);
    ...
    private static function buildUrl(WebsiteAnalysisApisSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WebsiteAnalysisApisError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WebsiteAnalysisApisResponse($fetched);
        }
    ...
    private function fail(WebsiteAnalysisApisContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WebsiteAnalysisApisContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
