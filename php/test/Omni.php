...
//    only - a typed WebsiteAnalysisApisContext there would make every ctx
...
final class WebsiteAnalysisApisOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WebsiteAnalysisApisContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WebsiteAnalysisApisSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WebsiteAnalysisApisResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WebsiteAnalysisApisError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WebsiteAnalysisApisResponse($clean['response']);
    ...
    public static function err_from_map($m): ?WebsiteAnalysisApisError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WebsiteAnalysisApisError(is_string($code) ? $code : '', $msg);
    }
