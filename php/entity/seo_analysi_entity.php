...
// WebsiteAnalysisApis SDK SeoAnalysi entity
...
class SeoAnalysiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SeoAnalysiEntity($this->_client, $opts);
    }

    /**
     * @param SeoAnalysi|array $args SeoAnalysi data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WebsiteAnalysisApisHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return SeoAnalysi|array The current SeoAnalysi data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of SeoAnalysi fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WebsiteAnalysisApisHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of SeoAnalysi fields).
     */
    ...
    /**
     * Load a single SeoAnalysi.
     *
     * @param SeoAnalysiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SeoAnalysiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return SeoAnalysi|array The loaded SeoAnalysi as an assoc-array at the
     *   SDK boundary; throws WebsiteAnalysisApisError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WebsiteAnalysisApisHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
