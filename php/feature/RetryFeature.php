...
// WebsiteAnalysisApis SDK retry feature
...
class WebsiteAnalysisApisRetryFeature extends WebsiteAnalysisApisBaseFeature
{
    ...
    public function init(WebsiteAnalysisApisContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WebsiteAnalysisApisContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WebsiteAnalysisApisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WebsiteAnalysisApisContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
