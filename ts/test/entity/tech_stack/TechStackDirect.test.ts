...
describe('TechStackDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new WebsiteAnalysisApisSDK({
  ...
  test('direct-list-tech_stack', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-tech_stack', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WEBSITE_ANALYSIS_APIS_TEST_TECH_STACK_ENTID']
