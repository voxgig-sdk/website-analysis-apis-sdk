...
describe('SeoAnalysiDirect', async () => {
  ...
  test('direct-load-seo_analysi', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-seo_analysi', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WEBSITE_ANALYSIS_APIS_TEST_SEO_ANALYSI_ENTID']
