...
const __1 = require("..");
const SDK_NAME = 'WebsiteAnalysisApisSDK';
...
const TEST_SEED = { "entity": { "performance": { "test01": { "id": "test01" } }, "screenshot": { "test01": { "id": "test01" } }, "seo": { "test01": { "id": "test01" } }, "seo_analysi": { "test01": { "id": "test01" } }, "ssl": { "test01": { "id": "test01" } }, "tech_stack": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+WebsiteAnalysisApisSDK\b/.test(code) ||
        /\bWebsiteAnalysisApisSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+WebsiteAnalysisApisSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/WebsiteAnalysisApisSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
