...
(0, node_test_1.describe)('ScreenshotEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'screenshot.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set WEBSITE_ANALYSIS_APIS_TEST_SCREENSHOT_ENTID JSON to run live');
            return;
        }
        ...
        let screenshot_ref01_data = Object.values(setup.data.existing.screenshot)[0];
        // LOAD
        const screenshot_ref01_ent = client.Screenshot();
        const screenshot_ref01_match_dt0 = {};
        const screenshot_ref01_data_dt0 = (await screenshot_ref01_ent.load(screenshot_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != screenshot_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/screenshot/ScreenshotTestData.json');
    ...
    let idmap = transform(['screenshot01', 'screenshot02', 'screenshot03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['WEBSITE_ANALYSIS_APIS_TEST_SCREENSHOT_ENTID'];
...
//# sourceMappingURL=ScreenshotEntity.test.js.map