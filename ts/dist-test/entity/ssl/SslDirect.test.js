...
(0, node_test_1.describe)('SslDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-ssl', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-ssl', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['WEBSITE_ANALYSIS_APIS_TEST_SSL_ENTID'];
