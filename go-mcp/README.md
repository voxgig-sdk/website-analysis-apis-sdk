# website-analysis-apis-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WebsiteAnalysisApis SDK as
two agent tools — `website-analysis-apis_list` and `website-analysis-apis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/website-analysis-apis-mcp)
make build

# 2. Provide credentials via the environment
export WEBSITE_ANALYSIS_APIS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user website-analysis-apis \
  -- /absolute/path/to/website-analysis-apis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./website-analysis-apis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// website-analysis-apis_list: first page of records
{ "entity": "seo" }
{ "entity": "seo", "query": { } }

// website-analysis-apis_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/website-analysis-apis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user website-analysis-apis \
     -- "$PWD"/dist/*/website-analysis-apis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `website-analysis-apis_list` and `website-analysis-apis_load` tools now appear
   in new sessions. Ask the agent to *"list seo using website-analysis-apis"*
   and it calls `website-analysis-apis_list` with `{"entity":"seo"}`.
...
```sh
export WEBSITE_ANALYSIS_APIS_APIKEY=sk_live_xxx            # API key
export WEBSITE_ANALYSIS_APIS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `website-analysis-apis_list` tool
...
### Call the `website-analysis-apis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `website-analysis-apis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `website-analysis-apis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEBSITE_ANALYSIS_APIS_APIKEY` | API key sent with every request. |
| `WEBSITE_ANALYSIS_APIS_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

performance | screenshot | seo | seo_analysi | ssl | tech_stack

### Smoke test via HTTP (raw JSON-RPC)

```sh
./website-analysis-apis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"website-analysis-apis_load","arguments":{"entity":"performance","query":{"id":1}}}}'
```
