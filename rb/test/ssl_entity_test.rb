...
class SslEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WEBSITEANALYSISAPIS_TEST_SSL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def ssl_basic_setup(extra)
  ...
  entid_env_raw = ENV["WEBSITEANALYSISAPIS_TEST_SSL_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "WEBSITEANALYSISAPIS_TEST_SSL_ENTID" => idmap,
    "WEBSITEANALYSISAPIS_TEST_LIVE" => "FALSE",
    "WEBSITEANALYSISAPIS_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["WEBSITEANALYSISAPIS_TEST_SSL_ENTID"])
