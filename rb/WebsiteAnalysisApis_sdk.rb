...
require_relative 'WebsiteAnalysisApis_types'
...
class WebsiteAnalysisApisSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WebsiteAnalysisApisHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WebsiteAnalysisApisFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WebsiteAnalysisApisUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = WebsiteAnalysisApisSpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WebsiteAnalysisApisError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WebsiteAnalysisApisError => err
    ...
    if fetched.is_a?(Hash)
      status = WebsiteAnalysisApisHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WebsiteAnalysisApisError.new(
        "graphql_error", "WebsiteAnalysisApisSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.SeoAnalysi.list / client.SeoAnalysi.load({ "id" => ... })
  def SeoAnalysi(data = nil)
    require_relative 'entity/seo_analysi_entity'
    SeoAnalysiEntity.new(self, data)
  end
  ...
  # Canonical facade: client.TechStack.list / client.TechStack.load({ "id" => ... })
  def TechStack(data = nil)
    require_relative 'entity/tech_stack_entity'
    TechStackEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WebsiteAnalysisApisSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
