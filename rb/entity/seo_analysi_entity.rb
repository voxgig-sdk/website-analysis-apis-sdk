...
class SeoAnalysiEntity
  ...
  def make
    opts = @_entopts.dup
    SeoAnalysiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SeoAnalysi, Hash] the current SeoAnalysi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SeoAnalysi fields)
  def match_get
  ...
  #
  # @param reqmatch [SeoAnalysiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.SeoAnalysi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [SeoAnalysi, Hash] the loaded SeoAnalysi; raises WebsiteAnalysisApisError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WebsiteAnalysisApisHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
