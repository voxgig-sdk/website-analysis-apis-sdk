...
local TechStackEntity = {}
TechStackEntity.__index = TechStackEntity


function TechStackEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TechStackEntity)
  self._name = "tech_stack"
  self._client = client
...
function TechStackEntity:get_name()
  return self._name
end


function TechStackEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TechStackEntity.new(self._client, opts)
end
...
function TechStackEntity:mark_deleted()
  self._deleted = true
end


function TechStackEntity:deleted()
  return true == self._deleted
end


function TechStackEntity:data_set(args)
  if args ~= nil then
...
function TechStackEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TechStackEntity:match_set(args)
  if args ~= nil then
...
function TechStackEntity:match_get()
...
function TechStackEntity:stream(action, args, callopts)
...
---@param reqmatch TechStackListMatch
---@param ctrl? table
---@return TechStack[]
---@return string? err
function TechStackEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TechStackEntity:_run_op(ctx, post_done)
  local utility = self._utility
