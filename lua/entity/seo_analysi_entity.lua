...
local SeoAnalysiEntity = {}
SeoAnalysiEntity.__index = SeoAnalysiEntity


function SeoAnalysiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SeoAnalysiEntity)
  self._name = "seo_analysi"
  self._client = client
...
function SeoAnalysiEntity:get_name()
  return self._name
end


function SeoAnalysiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SeoAnalysiEntity.new(self._client, opts)
end
...
function SeoAnalysiEntity:mark_deleted()
  self._deleted = true
end


function SeoAnalysiEntity:deleted()
  return true == self._deleted
end


function SeoAnalysiEntity:data_set(args)
  if args ~= nil then
...
function SeoAnalysiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SeoAnalysiEntity:match_set(args)
  if args ~= nil then
...
function SeoAnalysiEntity:match_get()
...
function SeoAnalysiEntity:stream(action, args, callopts)
...
---@param reqmatch SeoAnalysiLoadMatch
---@param ctrl? table
---@return SeoAnalysi
---@return string? err
function SeoAnalysiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SeoAnalysiEntity:_run_op(ctx, post_done)
  local utility = self._utility
