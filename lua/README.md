# WebsiteAnalysisApis Lua SDK



The Lua SDK for the WebsiteAnalysisApis API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Performance()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/website-analysis-apis-sdk/releases)),
...
Create an instance: `local seo_analysi = client:SeoAnalysi(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `headings` | `table` | Heading tags analysis |
| `images` | `table` |  |
...
Create an instance: `local tech_stack = client:TechStack(nil)`
...
```
lua/
├── website-analysis-apis_sdk.lua    -- Main SDK module
...
The main module (`website-analysis-apis_sdk`) exports the SDK constructor
