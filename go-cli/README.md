# website-analysis-apis-cli

boru-driven command-line client **and** interactive REPL for the WebsiteAnalysisApis
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/website-analysis-apis-cli)
make build

# 2. See usage (words, entities, env vars)
./website-analysis-apis-cli --help

# 3. Provide credentials once, via the environment
export WEBSITE_ANALYSIS_APIS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./website-analysis-apis-cli load 1 performance            # {id:1} shorthand
./website-analysis-apis-cli load '{id:1}' performance       # explicit match map

# 5. Override the API base URL for a single call
WEBSITE_ANALYSIS_APIS_BASE=https://api.example.com ./website-analysis-apis-cli load 1 performance

# 6. No arguments -> interactive REPL
./website-analysis-apis-cli
website-analysis-apis> load 1 performance
website-analysis-apis> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/website-analysis-apis-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/website-analysis-apis-cli load 1 performance
   ```
...
```sh
./website-analysis-apis-cli load 1 performance          # scalar shorthand for {id:1}
./website-analysis-apis-cli load '{id:1}' performance     # explicit match map
```
...
./website-analysis-apis-cli load 1 performance
```
...
Run with no arguments to open a REPL (prompt `website-analysis-apis>`). Each line is
evaluated as its own boru expression:

```text
$ ./website-analysis-apis-cli
website-analysis-apis> load 1 performance
website-analysis-apis> /help
website-analysis-apis> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `performance`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/website-analysis-apis-cli`. |
...
(`./website-analysis-apis-cli <expr>`) and interactively (the REPL), and expressions compose the
