// Command WebsiteAnalysisApis-cli is a CLI + REPL for the WebsiteAnalysisApis SDK,
...
const prompt = "website-analysis-apis"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "performance screenshot seo seo_analysi ssl tech_stack"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WEBSITE_ANALYSIS_APIS_APIKEY (api key), WEBSITE_ANALYSIS_APIS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WEBSITE_ANALYSIS_APIS_APIKEY carries the API key and
	// WEBSITE_ANALYSIS_APIS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WEBSITE_ANALYSIS_APIS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WEBSITE_ANALYSIS_APIS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWebsiteAnalysisApisSDK(opts)
