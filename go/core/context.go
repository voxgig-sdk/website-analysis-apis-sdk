...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*WebsiteAnalysisApisSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *WebsiteAnalysisApisError {
	return NewWebsiteAnalysisApisError(code, msg, ctx)
}
