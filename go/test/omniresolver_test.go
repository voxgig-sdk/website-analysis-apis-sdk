...
import (
	...
	omni "github.com/voxgig-sdk/website-analysis-apis-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WebsiteAnalysisApisError); is {
				out := map[string]any{
					"name":    "WebsiteAnalysisApisError",
...
func omniCtx(arg any, client *sdk.WebsiteAnalysisApisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WebsiteAnalysisApisSDK); is {
			client = live
