...
func TestScreenshotEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set WEBSITEANALYSISAPIS_TEST_SCREENSHOT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func screenshotBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WEBSITEANALYSISAPIS_TEST_SCREENSHOT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WEBSITEANALYSISAPIS_TEST_SCREENSHOT_ENTID"])
