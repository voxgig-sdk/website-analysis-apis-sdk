# WebsiteAnalysisApis SDK test runner
...
class WebsiteAnalysisApisTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WebsiteAnalysisApisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WebsiteAnalysisApisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WebsiteAnalysisApisTestRunner.getenv("WEBSITE_ANALYSIS_APIS_TEST_LIVE")
        override = WebsiteAnalysisApisTestRunner.getenv("WEBSITE_ANALYSIS_APIS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WebsiteAnalysisApisTestRunner.getenv(key)
        ...
        explain = WebsiteAnalysisApisTestRunner.getenv("WEBSITE_ANALYSIS_APIS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WebsiteAnalysisApisTestRunner._test_control is not None:
            return WebsiteAnalysisApisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WebsiteAnalysisApisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WebsiteAnalysisApisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WebsiteAnalysisApisTestRunner.load_test_control()
...
def load_env_local():
    WebsiteAnalysisApisTestRunner.load_env_local()


def env_override(m):
    return WebsiteAnalysisApisTestRunner.env_override(m)


def entity_data(v):
    return WebsiteAnalysisApisTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WebsiteAnalysisApisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WebsiteAnalysisApisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WebsiteAnalysisApisTestRunner.load_test_control()


def live_client_options():
    return WebsiteAnalysisApisTestRunner.live_client_options()


def live_delay_ms():
    return WebsiteAnalysisApisTestRunner.live_delay_ms()
