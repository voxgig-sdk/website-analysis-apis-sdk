...
class TestTechStackDirect:

    def test_should_direct_list_tech_stack(self):
        setup = _tech_stack_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tech_stack", "live" if setup["live"] else "unit")
        if _skip:
...
def _tech_stack_direct_setup(mockres):
    ...
    if live:
        ...
        client = WebsiteAnalysisApisSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
